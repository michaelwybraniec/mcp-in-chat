...
export class BoilerService {
   ...
   */
  async getAllBoilers(): Promise<Boiler[]> {
    try {
   ...
   * Get boiler by model
   */
  async getBoilerByModel(model: string): Promise<Boiler | null> {
    try {
      const boilers = await this.getAllBoilers();
      return boilers.find(boiler => 
        boiler.model.toLowerCase() === model.toLowerCase()
      ) || null;
    } catch (error) {
   ...
   */
  async getBoilersByManufacturer(manufacturer: string): Promise<Boiler[]> {
    try {
      const boilers = await this.getAllBoilers();
      const searchTerm = manufacturer.toLowerCase();
      return boilers.filter(boiler => 
        boiler.manufacturer.toLowerCase().includes(searchTerm)
      );
    } catch (error) {
   ...
   */
  async getBoilersByEfficiency(minEfficiency: string): Promise<Boiler[]> {
    try {
      const boilers = await this.getAllBoilers();
   ...
   */
  async getBoilersByPriceRange(minPrice: number, maxPrice: number): Promise<Boiler[]> {
    try {
      const boilers = await this.getAllBoilers();
      return boilers.filter(boiler => 
        boiler.price >= minPrice && boiler.price <= maxPrice
      );
    } catch (error) {
   ...
   */
  async searchBoilersByFeatures(features: string[]): Promise<Boiler[]> {
    try {
      const boilers = await this.getAllBoilers();
      return boilers.filter(boiler => 
        features.every(feature => 
          boiler.features.some(boilerFeature => 
            boilerFeature.toLowerCase().includes(feature.toLowerCase())
          )
        )
      );
    } catch (error) {
   ...
   */
  async getInventoryByModel(model: string): Promise<Inventory | null> {
    try {
   ...
   */
  async getAvailableInventory(): Promise<Inventory[]> {
    try {
      const inventory = await this.getAllInventory();
  ...
  async getLowStockInventory(): Promise<Inventory[]> {
    try {
      const inventory = await this.getAllInventory();
      return inventory.filter(item => 
        item.status === 'low_stock' || (item.quantity > 0 && item.quantity <= 5)
      );
    } catch (error) {
  ...
  async getOutOfStockInventory(): Promise<Inventory[]> {
    try {
      const inventory = await this.getAllInventory();
      return inventory.filter(item => 
        item.status === 'out_of_stock' || item.quantity === 0
      );
    } catch (error) {
   ...
   */
  async isBoilerAvailable(model: string, quantity: number = 1): Promise<boolean> {
    try {
   ...
   */
  async getBoilerWithInventory(model: string): Promise<{ boiler: Boiler | null; inventory: Inventory | null }> {
    try {
  ...
  async getRecommendedBoilers(criteria: {
    ...
    try {
      ...
      if (criteria.manufacturer) {
        ...
        boilers = boilers.filter(boiler => 
          boiler.manufacturer.toLowerCase().includes(manufacturer)
        );
      }
   ...
   * Compare boilers
   */
  async compareBoilers(models: string[]): Promise<{ [key: string]: Boiler | null }> {
    try {
      const boilers = await this.getAllBoilers();
      const result: { [key: string]: Boiler | null } = {};
      
      for (const model of models) {
        result[model] = boilers.find(boiler => 
          boiler.model.toLowerCase() === model.toLowerCase()
        ) || null;
      }
   ...
   */
  async getWarrantyInfo(model: string): Promise<any> {
    try {
