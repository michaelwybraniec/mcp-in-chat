    ...
    try {
      ...
      if (installation_date) {
        ...
        if (availableTechnicians.length > 0) {
          ...
          installationBooking = await technicianService.bookTechnician(
            customer_id,
 ...
 * Apply for finance
 */
router.post('/:customer_id/finance',
  authenticateApiKey,
  purchaseRateLimit,
  async (req: Request, res: Response) => {
    try {
 ...
 */
router.get('/available-payment-methods',
  authenticateApiKey,
  purchaseRateLimit,
  async (req: Request, res: Response) => {
    try {
...
async function getAlternativeBoilers(requestedModel: string, price: number): Promise<any[]> {
  try {
