...
export class WarrantyService {
     ...
     */
    async getWarrantyInfo(manufacturer, model) {
        try {
            const warranties = await this.getAllWarranties();
            const warranty = warranties.find(w => w.manufacturer.toLowerCase() === manufacturer.toLowerCase() &&
     ...
     */
    async getWarrantiesByManufacturer(manufacturer) {
        try {
            const warranties = await this.getAllWarranties();
            return warranties.filter(w => w.manufacturer.toLowerCase().includes(manufacturer.toLowerCase()));
        }
        catch (error) {
     ...
     */
    async isWarrantyActive(customerId) {
        try {
     ...
     */
    async getWarrantyExtensions(manufacturer, model) {
        try {
     ...
     */
    async calculateWarrantyExtensionCost(manufacturer, model, extensionType) {
        try {
     ...
     */
    async getWarrantyContact(manufacturer, model) {
        try {
            const warrantyInfo = await this.getWarrantyInfo(manufacturer, model);
     ...
     */
    async compareWarranties(models) {
        try {
            const comparison = {};
     ...
     */
    async validateWarrantyClaim(customerId, issue, serviceDate) {
        try {
    ...
    parseWarrantyDuration(duration) {
