...
export class PaymentService {
     ...
     * Process payment
     */
    async processPayment(paymentInfo, orderId) {
        try {
     ...
     */
    async processCreditCardPayment(cardNumber, expiryDate, cvv, amount, orderId) {
        const paymentInfo = {
            method: 'credit_card',
            amount,
            currency: 'GBP',
     ...
     */
    async processBankTransfer(accountNumber, sortCode, amount, orderId) {
        const paymentInfo = {
     ...
     */
    async processCashPayment(amount, orderId) {
        const paymentInfo = {
            method: 'cash',
            amount,
            currency: 'GBP'
     ...
     * Apply for finance
     */
    async applyForFinance(customerId, amount, term = 24) {
        try {
            // Simulate processing delay
            await this.simulateProcessingDelay();
     ...
     */
    async processFinancePayment(applicationId, amount, orderId) {
        const paymentInfo = {
            method: 'finance',
            amount,
            currency: 'GBP'
    ...
    calculateMonthlyPayment(amount, term) {
        const interestRate = this.getInterestRate(amount, term);
