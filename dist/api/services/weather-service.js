...
export class WeatherService {
     ...
     */
    async getWeatherForecast(date, location) {
        try {
            const weatherData = await this.getAllWeatherData();
            const forecast = weatherData.find(w => w.date === date && w.location.toLowerCase() === location.toLowerCase());
     ...
     */
    async getWeatherForecastRange(startDate, endDate, location) {
        try {
            const weatherData = await this.getAllWeatherData();
     ...
     */
    async isWeatherSuitableForMaintenance(date, location) {
        try {
     ...
     */
    async getMaintenanceWeatherRecommendation(preferredDate, location, flexibilityDays = 7) {
        try {
            ...
            const alternativeDates = await this.findSuitableDates(preferredDate, location, flexibilityDays);
            return {
                date: preferredDate,
                suitable: false,
     ...
     */
    async getOptimalMaintenanceDates(location, days = 30) {
        try {
            const today = new Date();
            const endDate = new Date(today.getTime() + days * 24 * 60 * 60 * 1000);
            const forecasts = await this.getWeatherForecastRange(today.toISOString().split('T')[0], endDate.toISOString().split('T')[0], location);
            return forecasts
    ...
    async getWeatherAlerts(location) {
        try {
            const weatherData = await this.getAllWeatherData();
            const locationData = weatherData.filter(w => w.location.toLowerCase() === location.toLowerCase());
            const alerts = [];
     ...
     */
    async getWeatherSummary(location, days = 7) {
        try {
            const today = new Date();
            const endDate = new Date(today.getTime() + days * 24 * 60 * 60 * 1000);
            const forecasts = await this.getWeatherForecastRange(today.toISOString().split('T')[0], endDate.toISOString().split('T')[0], location);
    ...
    async findSuitableDates(preferredDate, location, flexibilityDays) {
        try {
            ...
            const forecasts = await this.getWeatherForecastRange(startDate.toISOString().split('T')[0], endDate.toISOString().split('T')[0], location);
            return forecasts
                .filter(f => f.maintenance_suitable)
                .map(f => f.date)
                .sort();
    ...
    calculateWeatherScore(forecast) {
        let score = 0;
